...
function selectedControlledOption(element) {
  return Array.from(element?.querySelectorAll?.(CONTROLLED_OPTION_SELECTOR) ?? [])
    .find((option) => option.getAttribute("aria-selected")?.trim().toLowerCase() === "true"
      ...
      || ["checked", "selected", "active", "current", "highlighted", "focused", "on"].includes(option.getAttribute("data-state")?.trim().toLowerCase())
...
function hasSelectedOptionClass(element) {
  return Array.from(element.classList ?? [])
    .some((className) => {
      ...
      return /^(?:is-|ps-|ui-|nyu-)?(?:selected|active|current|checked|on)$/.test(normalizedClassName)
        || /^(?:is|ps|ui|nyu)[-_](?:selected|active|current|checked|on)[-_]option$/.test(normalizedClassName)
        || /^(?:selected|active|current|checked|on)[-_]option$/.test(normalizedClassName)
        || /^option[-_](?:selected|active|current|checked|on)$/.test(normalizedClassName);
    });
}
...
function firstNameLikeAttribute(element) {
  for (const attributeName of ["title", "aria-label", "data-value", "data-label", "data-text", "data-display", "data-display-name", "data-name"]) {
    const value = element.getAttribute(attributeName)?.trim();
