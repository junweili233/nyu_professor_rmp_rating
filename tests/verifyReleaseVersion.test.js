...
describe("release version verifier", () => {
  it("passes when package, runtime, source manifest, dist manifest, and tag align", async () => {
    await expect(verifyReleaseVersion({ distDir: "dist", releaseTag: "v0.1.3" })).resolves.toMatchObject({
      ok: true,
      packageVersion: "0.1.3",
      runtimeVersion: "0.1.3",
      sourceManifestVersion: "0.1.3",
      distManifestVersion: "0.1.3",
      expectedTag: "v0.1.3",
  ...
  it("fails when the built dist manifest is stale", async () => {
    ...
    await expect(verifyReleaseVersion({ distDir: staleDist, releaseTag: "v0.1.3" })).rejects.toThrow(
      "dist manifest version 0.1.1 does not match package.json 0.1.3",
    );
  ...
  it("fails when the release tag does not match package.json", async () => {
    await expect(verifyReleaseVersion({ distDir: "dist", releaseTag: "v0.1.1" })).rejects.toThrow(
      "release tag v0.1.1 does not match package.json version v0.1.3",
    );
  });
});
