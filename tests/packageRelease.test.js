...
describe("release packager", () => {
  it("parses dist, tag, output directory, and dry-run arguments", () => {
    ...
    expect(packageReleaseArgs(["dist", "v0.1.7", "release", "--dry-run"])).toEqual({
      distDir: "dist",
      releaseTag: "v0.1.7",
      outDir: "release",
      dryRun: true,
  ...
  it("dry-runs the expected release asset without invoking compression", async () => {
    ...
    await expect(packageRelease({ distDir: "dist", releaseTag: "v0.1.7", dryRun: true, execFileImpl })).resolves.toMatchObject({
      ok: true,
      expectedTag: "v0.1.7",
      expectedAssetName: "nyu-albert-rmp-ratings-v0.1.7.zip",
      dryRun: true,
    });
  ...
  it("builds the PowerShell compression command from verified release metadata", async () => {
    ...
    const result = await packageRelease({ distDir: "dist", releaseTag: "v0.1.7", outDir: "release", execFileImpl });

    expect(result.expectedAssetName).toBe("nyu-albert-rmp-ratings-v0.1.7.zip");
    expect(result.command).toContain("Compress-Archive");
    expect(result.command).toContain("nyu-albert-rmp-ratings-v0.1.7.zip");
