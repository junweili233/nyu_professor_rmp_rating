...
describe("popup diagnostics verifier", () => {
  it("passes when copied popup diagnostics prove current segmented Albert cards", () => {
    expect(verifyPopupDiagnostics([
      ...
      "Page status: Albert connected v0.1.5: 4 rating roots, 4 cards, 4 segmented quick views, 1 radar map, 4 Albert cells checked, 4 trailing rating columns, layout OK",
    ].join("\n"))).toMatchObject({
      ok: true,
  ...
  it("fails when Albert reports missing content version and no quick views", () => {
    expect(() => verifyPopupDiagnostics([
      "NYU Albert RMP Ratings diagnostics",
      "Build v0.1.5 | Albert missing | 4 cards | 0 quick views | 4 cells | 0 rating columns",
      "Page status: Albert connected; old squeezed card layout detected.",
    ].join("\n"))).toThrow("Albert content version missing does not match expected 0.1.5");
    try {
      verifyPopupDiagnostics("Build v0.1.5 | Albert missing | 4 cards | 0 quick views | 4 cells | 0 rating columns");
    } catch (error) {
      expect(error.result).toMatchObject({
        ...
        quickViewCount: 0,
        failures: [
          "Albert content version missing does not match expected 0.1.5",
